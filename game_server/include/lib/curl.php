...
class curl
{
	public function __construct()
	{
		$this->setting_list = array("active" => true, "max_connections" => 10, "debug" => false);
	...
	public function update()
	{
		...
		while(count($this->connection_list) < $this->setting_list['max_connections'] && count($this->job_list) > 0)
		{
			...
			$url = $job['request']->get_option(CURLOPT_URL);

			// rebuild the URL
			$url = reconstruct_url($url);
			$host = parse_url($url, PHP_URL_HOST);

			$job['request']->set_option(CURLOPT_URL, $url);
		...
		while($item = curl_multi_info_read($this->mc))
		{
			...
			curl_close($handle);
var_dump(count($this->connection_list));
